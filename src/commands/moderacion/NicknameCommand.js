...
module.exports = class NicknameCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    ...
    if (!args[0]) return message.channel.send("Tenes que mencionar a un miembro `!nickname @miembro nickname`");
    if (!mentionedMember) return message.channel.send("Ese miembro no esta en el server")
    if (!nickName) return message.channel.send("Tenes que definir un nuevo nick para el miembro. `!nickname @miembro nickname`");
    if (!mentionedMember.kickable) return message.channel.send("El rol del miembro es mas alto que el mio");
